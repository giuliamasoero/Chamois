...
absolute_all_w_t_503_449 <- slidingwin(
...
save(absolute_all_w_t_503_449, file = "output/absolute_all_w_t_503_449.rda")
```


```{r}
load(file = "output/absolute_all.rda")
load(file = "output/absolute_all_w_t_503_449.rda")
absolute_all$combos

absolute_all_w_t_503_449$combos
# provides a summary of all tested climate window hypotheses,
# with the key statistics from the best model included
...
head(subset(absolute_all[[2]]$Dataset, WindowClose < 100))
head(absolute_all_w_t_503_449[[2]]$Dataset)
head(subset(absolute_all_w_t_503_449[[2]]$Dataset, WindowClose < 100))
head(subset(absolute_all_w_t_503_449[[1]]$Dataset, WindowClose < 100))
...
# The best time window is between day 756 and 756- AIC= --186.7308
summary(absolute_all_w_t_503_449[[2]]$BestModel)
# the results from the best supported model for each hypothesis

head(absolute_all_w_t_503_449[[2]]$BestModel)
# Finally, the object BestModel stores the values of the climate signal
# that was best supported by the data and the row numbers correspond
# to the original row numbers of the Biol dataset.
...
# plot results from best model
plotall(
  dataset = absolute_all_w_t_503_449[[2]]$Dataset,
  bestmodel = absolute_all_w_t_503_449[[2]]$BestModel,
  bestmodeldata = absolute_all_w_t_503_449[[2]]$BestModelData
)


plotwin(dataset = absolute_all_w_t_503_449[[2]]$Dataset)
plotdelta(dataset = absolute_all[[2]]$Dataset)
plotdelta(dataset = absolute_all[[1]]$Dataset)
plotdelta(dataset = absolute_all_w_t_503_449[[2]]$Dataset)
plotdelta(dataset = absolute_all_w_t_503_449[[1]]$Dataset)
...
ggplot(ch_biom, aes(x = year, y = t_503_449)) +
  geom_point(shape = 1) +
  # Use hollow circles
  geom_smooth(method = "lm") +
  # Add linear regression line (by default includes 95% confidence region)
  scale_x_continuous(breaks = c(1992, 1997, 2002, 2007, 2013, 2018)) +
  xlab("Year") +
  ylab("Mean temperature of the best climatic window for NEW stage (°C)") +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.background = element_blank(),
