...
clim2 <- read.csv("data/swiss_weather_acquarossa.csv", stringsAsFactors = T, na = c("", "NA"))
colnames(clim2) <- c("station", "meteosw", "Temp", "Temp2", "date", "year", "month")
...
clim2$date_ymd <- as.Date(clim2$date, "%Y/%m/%d")
...
# create model with ABSOLUTE time window with temperature for ALWAYS period
# for the period 1 Dec(-2) - 31May(-1)
ch_mass_sw2 <- slidingwin(
    baseline = ch_basemod,
    xvar = list(Temp = clim2$Temp),
    type = "absolute",
    refday = c(24, 9),
    range = c(661, 0),
    stat = "mean",
    cdate = clim2$date_ymd,
    bdate = ch_biom15$date_ymd,
    func = c("lin", "quad"),
    cmissing = FALSE,
    cinterval = "day"
)
save(ch_mass_sw2, file = "output/ch_mass_sw_sc_clim2.rda")
```
...
load(file = "output/ch_mass_sw_sc_clim2.rda")
```
...
ch_biom15$temp_545_449 <- ch_mass_sw2[[2]]$BestModelData$climate
```
Dates of this window
```{r}
as.Date("2022/09/24", format = "%Y/%m/%d") - 545
...
save(ch_mass_sw2_lm2, file = "output/ch_mass_sw2_lm2.rda")
```
...
load("output/ch_mass_sw2_lm2.rda")
ch_mass_sw_lm2$combos
ch_mass_sw2_lm2$combos
...
```{r Fig. 2, warning = F, dev = c("png","tiff"), fig.height = 9, fig.width = 4, fig.cap = "Fig. 2 - SOMETHING"}
grid.arrange(
    plot_yr_temp, plot_yr_bm, resid_qlm1,
    nrow = 3
)
```
