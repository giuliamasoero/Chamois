...
linera_model <- lm(weight~year, data = data) #making the linear model

anova(linera_model)

summary(linera_model)
...
m3_scale<- lm(data$weight ~ sex + (data$altitude_scale) * (data$temp.absolute_juvenile_scale) + I(data$temp.absolute_juvenile_scale)^2) + (data$temp.absolute_kid_scale) + I(data$temp.absolute_kid_scale)^2) + (data$temp.absolute_gestation_scale) + I(data$temp.absolute_gestation_scale)^2), data = data)
...
m_result1 <- (lm (data$weight ~ sex + data$altitude_scale + data$temp.absolute_juvenile_scale * data$temp.absolute_kid_scale * data$temp.absolute_gestation_scale + I(data$temp.absolute_juvenile_scale)^2 +I(data$temp.absolute_kid_scale)^2 + I(data$temp.absolute_gestation_scale)^2) data = data)
...
randomized_gestation<-randwin(repeats = 10, 
                                         baseline = lm(data$weight ~ sex + altitude, data = data),
                                         xvar = list(Temp=clim$Temp),
                                          type = "absolute",
                                          refday = c(24,9),
